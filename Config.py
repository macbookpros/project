import numpy
from mpi4py import MPI
class Config:
    A_file = None
    B_file = None
    C_file = None
    grid_comm = None
    row_comm = None
    col_comm = None
    dim = []
    coords = []
    A_dims = []
    B_dims = []
    C_dims = []
    matrix_size = 0
    local_dims = []
    local_size = 0
    my_row = 0
    my_col = 0
    world_rank=None
    WORLD_SIZE=None
    grid_rank=None
    A=None
    A_tmp=None
    B=None
    C=None




    def compute_fox(self):
        comm = MPI.COMM_WORLD
        self.A_tmp = numpy.ndarray(int(self.local_dims[0] *self.local_dims[0]*8))
        source = (self.my_row + 1) % self.dim[0]
        dest = (self.my_row + self.dim[0] - 1) % self.dim[0]


        for i in range(0,int(self.dim[0])):
            root = int((self.my_row + i) % self.dim[0])
            if int(root) == int(self.my_col):

                self.A = comm.bcast(self.A, root)
                self.multiply_matrix(self.A)

            else:
                self.A_tmp = comm.bcast(self.A_tmp, root)
                self.multiply_matrix(self.A_tmp)

        comm.Sendrecv_replace(self.B,dest,0,source,0,status=None)


    def multiply_matrix(self, Amatrix):

        for i in range(0, int(self.local_dims[0])):
            for k in range(0, int(self.local_dims[0])):
                for j in range(0, int(self.local_dims[0])):

                    #print(self.C[int(i*self.local_dims[0]+j)])
                    #print(Amatrix[int(i*self.local_dims[0]+k)]*self.B[int(k*self.local_dims[0]+j)])
                    self.C[int(i*self.local_dims[0]+j)] += Amatrix[int(i*self.local_dims[0]+k)]*self.B[int(k*self.local_dims[0]+j)]


